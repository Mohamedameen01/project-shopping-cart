...
router.post('/place-order', async(req, res) => {
  let products = await productFuncs.getCartProductsList(req.session.user._id)
  let total = await productFuncs.getTotalAmount(req.session.user._id)
  
  productFuncs.placeOrder(req.body, products, total).then((response) => {
...
router.get('/view-selected-order/', (req, res) => {
  ...
  productFuncs.getSelectedOrder(req.query.id).then(response => {
    console.log(response);
    res.render('user/view-selected-order', {products: response, user: req.session.user})
  })
})
